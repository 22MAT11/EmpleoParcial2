...
def register_stats_callbacks(app):
    ...
    def actualizar_kpis(año, ciudad, genero):
        est = calcular_estadisticas(ciudad, año, genero)
    ...
    def actualizar_gauss(año, ciudad, genero):
        return figura_gauss(ciudad, año, genero).to_dict()
    ...
    def actualizar_tendencia(ciudad, genero):
        if genero == "Ambos":
            return _FIGURES_CACHE.get(f"tendencia_{ciudad}", figura_tendencia(ciudad)).to_dict()
        return figura_tendencia(ciudad, genero).to_dict()
    ...
    def actualizar_ranking(año, n_clicks, genero):
        is_top5 = n_clicks and n_clicks % 2 == 1
        if genero == "Ambos":
            cache_key = f"ranking_{año}_top5" if is_top5 else f"ranking_{año}"
            fig = _FIGURES_CACHE.get(cache_key, figura_ranking(año, top_n=5 if is_top5 else None))
        else:
            fig = figura_ranking(año, top_n=5 if is_top5 else None, genero=genero)
        return fig.to_dict(), "[Ver todas]" if is_top5 else "[Top 5]"
