...
def figura_gauss(ciudad: str, año: int, genero: str = "Ambos") -> go.Figure:
    idx = AÑOS.index(año)
    mu_ciudad = EMPLEO_BASE[ciudad][idx]
    mu_ciudad = ajustar_tasa_por_genero(mu_ciudad, ciudad, genero)
    ...
    valores_año = [ajustar_tasa_por_genero(EMPLEO_BASE[c][idx], c, genero) for c in CIUDADES if EMPLEO_BASE[c][idx] is not None]
    ...
    fig.update_layout(
        title=dict(
            text=f"<b>Distribución Normal</b> — {ciudad} vs Colombia<br>"
