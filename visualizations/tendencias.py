...
def figura_tendencia(ciudad: str, genero: str = "Ambos") -> go.Figure:

    tasas = [ajustar_tasa_por_genero(t, ciudad, genero) for t in EMPLEO_BASE[ciudad]]
    ...
    nac = [calcular_media_nacional(a, genero)[0] for a in AÑOS]
    fig.add_trace(
        go.Scatter(
            x=AÑOS,
            y=nac,
    ...
    fig.update_layout(
        title=dict(
            text=f"Tendencia — <b style='color:{color}'>{ciudad}</b> (2021-2026)",
