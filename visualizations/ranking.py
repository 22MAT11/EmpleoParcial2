...
def figura_ranking(año: int, top_n: Optional[int] = None, genero: str = "Ambos") -> go.Figure:

    idx = AÑOS.index(año)
    datos = [(c, ajustar_tasa_por_genero(EMPLEO_BASE[c][idx], c, genero), CIUDADES[c]["region"]) for c in CIUDADES if EMPLEO_BASE[c][idx] is not None]
    ...
    fig.update_layout(
        title=dict(
            text="Ranking Empleabilidad", font=dict(color=TEXT_COLOR, size=12),
            y=0.98,
            yanchor="top",
        ),
        xaxis=dict(
